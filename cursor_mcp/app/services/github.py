...
class GitHubService:
    ...
    def __init__(self, token: str = settings.github_token, repo_name: str = settings.github_repository):
        try:
            ...
            logger.info(f"Initialized GitHub service for repository: {repo_name}")
        except Exception as e:
            logger.warning(f"Failed to authenticate with GitHub: {str(e)}")
    ...
    def _fetch_rules_sync(self, technology: str) -> List[Rule]:
        ...
        try:
            ...
            contents = self.repo.get_contents(f"rules/{technology}")
